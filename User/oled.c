...
uint8_t OLED_DMA_StateFlag = 0;              //0空闲 1占用    表示8bit的数组是否空闲
uint8_t OLED_TransBuffer[8][1026];

void OLED_ShowFrame_DMA(){
  static Struct_I2C_TransData I2C_TransData;

  uint8_t Free_Buffer_Num = 0;
  Free_Buffer_Num = Find_Free_Buffer(OLED_DMA_StateFlag, 8);
  if(Free_Buffer_Num){
    I2C_TransData.Length = OLED_COLUMN * OLED_PAGE;
    I2C_TransData.Num    = Free_Buffer_Num;                     //记录下正在传输的数组
    I2C_TransData.Data_Point = &OLED_TransBuffer[Free_Buffer_Num - 1][0];
    
    Set_Busy_BufferBit(&OLED_DMA_StateFlag, Free_Buffer_Num);

    memcpy(OLED_TransBuffer[Free_Buffer_Num - 1], &OLED_GRAM[0][0], OLED_COLUMN * OLED_PAGE);
    I2C1_Add_TransData(I2C_TransData);
  }
  else{
    //找不到空闲的就丢弃这一帧
    return;
...
void OLED_Init()
{
  ...
  OLED_SendCmd(0x22);
  OLED_SendCmd(0x00);         //不知道为什么页起始地址得偏移一页，正常来说是0开始
  OLED_SendCmd(0x07);          
...
void OLED_ShowFrame()
{
  ...
  //水平寻址模式
  // OLED_SendCmd(0x40);
  // OLED_Send(&OLED_GRAM[0][0], (uint16_t)OLED_COLUMN * OLED_PAGE);
}
 ...
 * @param color  颜色
 */
void OLED_DrawFilledRoundRect(uint8_t x, uint8_t y, uint8_t w, uint8_t h, uint8_t r, OLED_ColorMode color)
{
    if (r > w / 2 || r > h / 2)  // 限制圆角半径，防止超出矩形尺寸
        r = (w < h ? w : h) / 2;

    // 1. 先填充矩形的主体（去掉四个角的区域）
    for (uint8_t i = y + r; i < y + h - r; i++)  
    {
    ...
    {
        OLED_DrawLine(x + r, y + i, x + w - r - 1, y + i, color);  // 上方矩形
        OLED_DrawLine(x + r, y + h - i - 1, x + w - r - 1, y + h - i - 1, color);  // 下方矩形
    }

    // 3. 填充四个圆角
    for (uint8_t dy = 0; dy < r; dy++)
    {
        uint8_t dx = (uint8_t)(sqrt(r * r - dy * dy));  // 计算水平偏移量，确保圆角平滑

        // 左上角
        OLED_DrawLine(x + r - dx, y + r - dy - 1, x + r - 1, y + r - dy - 1, color);
        // 右上角
        OLED_DrawLine(x + w - r, y + r - dy - 1, x + w - r + dx - 1, y + r - dy - 1, color);
        // 左下角
        OLED_DrawLine(x + r - dx, y + h - r + dy, x + r - 1, y + h - r + dy, color);
        // 右下角
        OLED_DrawLine(x + w - r, y + h - r + dy, x + w - r + dx - 1, y + h - r + dy, color);
    }
}
